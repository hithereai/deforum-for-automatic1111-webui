...
def anim_frame_warp(prev_img_cv2, args, anim_args, keys, frame_idx, depth_model=None, depth=None, device='cuda', half_precision = False):
    ...
    if anim_args.use_depth_warping:
        if depth is None and depth_model is not None:
            depth = depth_model.predict(prev_img_cv2, half_precision)
...
def transform_image_3d(device, prev_img_cv2, depth_tensor, rot_mat, translate, anim_args, keys, frame_idx, depth_offset=2):
    h, w = prev_img_cv2.shape[0], prev_img_cv2.shape[1]

    aspect_ratio = keys.aspect_ratio_series[frame_idx] if not anim_args.aspect_ratio_use_old_formula else float(w) / float(h)
    near, far, fov_deg = keys.near_series[frame_idx], keys.far_series[frame_idx], keys.fov_series[frame_idx]
    
    persp_cam_old = p3d.FoVPerspectiveCameras(near, far, aspect_ratio, fov=fov_deg, degrees=True, device=device)
    persp_cam_new = p3d.FoVPerspectiveCameras(near, far, aspect_ratio, fov=fov_deg, degrees=True, R=rot_mat, T=torch.tensor([translate]), device=device)

    y, x = torch.meshgrid(torch.linspace(-1., 1., h, dtype=torch.float32, device=device), torch.linspace(-1., 1., w, dtype=torch.float32, device=device))
    z = torch.ones_like(x) if depth_tensor is None else (torch.as_tensor(depth_tensor, dtype=torch.float32, device=device) + depth_offset) / (torch.max(depth_tensor) + depth_offset)
    
    xyz_old_world = torch.stack((x.flatten(), y.flatten(), z.flatten()), dim=1)
    xyz_old_cam_xy, xyz_new_cam_xy = persp_cam_old.get_full_projection_transform().transform_points(xyz_old_world)[:, :2], persp_cam_new.get_full_projection_transform().transform_points(xyz_old_world)[:, :2]
    offset_xy = xyz_new_cam_xy - xyz_old_cam_xy 
    identity_2d_batch = torch.tensor([[1., 0., 0.], [0., 1., 0.]], device=device).unsqueeze(0)
    coords_2d = torch.nn.functional.affine_grid(identity_2d_batch, [1, 1, h, w], align_corners=False)
    offset_coords_2d = coords_2d - torch.reshape(offset_xy, (h, w, 2)).unsqueeze(0)
